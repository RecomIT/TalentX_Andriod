import 'package:intl/intl.dart';
class UserProfile {
  String empId;
  BasicInfo basicInfo;
  ContactInfo contactInfo;
  PersonalInfo personalInfo;
  String photo;

  UserProfile({this.empId, this.basicInfo, this.contactInfo, this.personalInfo, this.photo});

  UserProfile.fromJson(Map<String, dynamic> json) {
    empId = json['user_id'].toString();
    basicInfo =  new BasicInfo.fromJson(json);
    contactInfo = new ContactInfo.fromJson(json);
    personalInfo =  new PersonalInfo.fromJson(json);

    // basicInfo = json['BasicInfo'] != null ? new BasicInfo.fromJson(json['BasicInfo']) : null;
    // contactInfo = json['ContactInfo'] != null ? new ContactInfo.fromJson(json['ContactInfo']) : null;
    // personalInfo = json['PersonalInfo'] != null ? new PersonalInfo.fromJson(json['PersonalInfo']) : null;
    //photo =  getBdAvaterImage();
    photo = json['profile_photo'].toString() == 'null' ? getBdAvaterImage(): json['profile_photo'].toString();
  }

  String getBdAvaterImage() {
    // final bytes = Io.File('/assets/images/bd_avatar.png').readAsBytesSync();
    // String img64 = base64Encode(bytes);
    String BdImage='';
    return BdImage;
  }

  Map<String, dynamic> toJson() {
    final Map<String, dynamic> data = new Map<String, dynamic>();
    data['EmpId'] = this.empId;
    if (this.basicInfo != null) {
      data['BasicInfo'] = this.basicInfo.toJson();
    }
    if (this.contactInfo != null) {
      data['ContactInfo'] = this.contactInfo.toJson();
    }
    if (this.personalInfo != null) {
      data['PersonalInfo'] = this.personalInfo.toJson();
    }
    data['Photo'] = this.photo;
    return data;
  }
}

class BasicInfo {
  String name;
  String first_name;
  String middle_name;
  String last_name;
  String shortName;
  String jobLevel;
  String department;
  String subDepartment;
  String sSubDepartment;
  String workLocation;
  String designation;
  String division;
  String supervisor;
  String manager;
  String headofDepartment;
  String leadManagement;
  String hRAuthority;
  String dateofJoining;
  String dateofConfirmation;
  String dateOfBirth;
  String fingerId;
  String refId;
  String dateofSeperation;
  String grade;
  String code;
  String employeeRoleName;

  BasicInfo(
      {this.code,
      this.employeeRoleName,
      this.grade,
      this.name,
      this.first_name,
      this.middle_name,
      this.last_name,
      this.shortName,
      this.jobLevel,
      this.department,
      this.subDepartment,
      this.sSubDepartment,
      this.workLocation,
      this.designation,
      this.division,
      this.supervisor,
      this.manager,
      this.headofDepartment,
      this.leadManagement,
      this.hRAuthority,
      this.dateofJoining,
      this.dateofConfirmation,
      this.dateOfBirth,
      this.fingerId,
      this.refId,
      this.dateofSeperation});

  BasicInfo.fromJson(Map<String, dynamic> json) {

    first_name = json['first_name'];
    middle_name = json['middle_name'];
    last_name = json['last_name'];

    String fName = json['first_name'].toString() == 'null' ? '': json['first_name'].toString().trim();
    String mName = json['middle_name'].toString() == 'null' ? '': json['middle_name'].toString().trim();
    String lName = json['last_name'].toString() == 'null' ? '': json['last_name'].toString().trim();
    // print(fName);
    // print(mName);
    // print(lName);

    name= fName;
    if(mName.isNotEmpty) {
      name = name + ' ' + mName;
    }
    if (lName.isNotEmpty) {
      name=  name + ' ' + lName;
    };
    //print(name);

    //name = '$fName $mName $lName';


    shortName = json['first_name'];
    code=json['code'];
    jobLevel = json['category_id'].toString() == 'null' ? 'N/A': json['category_id'].toString();
    department = json['department_name'].toString() == 'null' ? 'N/A': json['department_name'].toString();
    subDepartment = 'N/A';
    sSubDepartment = 'N/A';
    division = json['division_name'].toString() == 'null' ? 'N/A': json['division_name'].toString();
    workLocation = json['location_name'].toString() == 'null' ? 'N/A': json['location_name'].toString();
    designation = json['designation_name'].toString() == 'null' ? 'N/A': json['designation_name'].toString();
    grade = json['grade_name'].toString() == 'null' ? 'N/A': json['grade_name'].toString();

    supervisor = json['supervisor_name'].toString() == 'null' ? 'N/A': json['supervisor_name'].toString();
    employeeRoleName = json['employee_role_name'].toString() == 'null' ? 'N/A': json['employee_role_name'].toString();
    manager =  'N/A';
    headofDepartment =  'N/A';
    leadManagement =  'N/A';
    hRAuthority =  'N/A';

    //can not be parsed as other properties due to unsupported string formate
    //dateofJoining = json['joining_date'].toString() == 'null' ? 'N/A': json['joining_date'].toString();

    var dateFormatter = new DateFormat('dd-MMM-yyyy');
    var doc= (json['confirmation_date']);
    var dob=(json['dob']);
    var doj=(json['joining_date']);
    var dos=(json['resigned_date']);

    dateofConfirmation= (doc)==null ? 'N/A' : dateFormatter.format(DateTime.parse(doc));
    dateOfBirth = (dob)==null ? 'N/A' : dateFormatter.format(DateTime.parse(dob));
    dateofJoining=(doj)==null ? 'N/A' : dateFormatter.format(DateTime.parse(doj));
    dateofSeperation = (dos)==null ? 'N/A' :  dateFormatter.format(DateTime.parse(dos));

    //dateofConfirmation = json['confirmation_date'].toString() == 'null' ? 'N/A': json['confirmation_date'].toString();
    //dateOfBirth = json['dob'].toString() == 'null' ? 'N/A': json['dob'].toString();
    //dateofSeperation = json['resigned_date'].toString() == 'null' ? 'N/A': json['resigned_date'].toString();
    fingerId = 'N/A';
    refId ='N/A';

  }

  Map<String, dynamic> toJson() {
    final Map<String, dynamic> data = new Map<String, dynamic>();
    data['first_name'] = this.first_name;
    data['middle_name'] = this.middle_name;
    data['last_name'] = this.last_name;
    data['name'] = this.name;
    data['ShortName'] = this.shortName;
    data['code'] = this.code;

    data['category_id'] = this.jobLevel;
    data['department_id'] = this.department;
    data['SubDepartment'] = this.subDepartment;
    data['SSubDepartment'] = this.sSubDepartment;

    data['location_id'] = this.workLocation;
    data['designation_id'] = this.designation;
    data['supervisor_id'] = this.supervisor;
    data['Manager'] = this.manager;
    data['HeadofDepartment'] = this.headofDepartment;
    data['LeadManagement'] = this.leadManagement;
    data['HRAuthority'] = this.hRAuthority;
    data['joining_date'] = this.dateofJoining;
    data['confirmation_date'] = this.dateofConfirmation;
    data['dob'] = this.dateOfBirth;
    data['FingerId'] = this.fingerId;
    data['RefId'] = this.refId;
    data['resigned_date'] = this.dateofSeperation;
    return data;
  }
}

class ContactInfo {
  String personalEmail;
  String officialEmail;
  String personalPhoneNo;
  String officialPhoneNo;
  String emergencyContactPerson;
  String emergencyContactRelation;
  String emergencyContactNumber;
  String emergencyContactAddress;
  String presentAddress;
  String permanentAddress;

  ContactInfo(
      {this.personalEmail,this.officialEmail,this.personalPhoneNo,this.officialPhoneNo,this.emergencyContactPerson,this.emergencyContactRelation,this.emergencyContactNumber,this.emergencyContactAddress,this.presentAddress,this.permanentAddress});

  ContactInfo.fromJson(Map<String, dynamic> json) {
    personalEmail = json['alt_email'].toString() == 'null' ? 'N/A': json['alt_email'].toString();
    officialEmail = json['email'].toString() == 'null' ? 'N/A': json['email'].toString();
    personalPhoneNo = json['alt_phone'].toString() == 'null' ? 'N/A': json['alt_phone'].toString();
    officialPhoneNo = json['phone'].toString() == 'null' ? 'N/A': json['phone'].toString();
    emergencyContactPerson = json['emergency_contact_name'].toString() == 'null' ? 'N/A': json['emergency_contact_name'].toString();
    emergencyContactRelation = json['emergency_contact_relation'].toString() == 'null' ? 'N/A': json['emergency_contact_relation'].toString();
    emergencyContactNumber = json['emergency_contact_phone'].toString() == 'null' ? 'N/A': json['emergency_contact_phone'].toString();
    emergencyContactAddress = json['emergency_contact_address'].toString() == 'null' ? 'N/A': json['emergency_contact_address'].toString();
    presentAddress = json['present_address'].toString() == 'null' ? 'N/A': json['present_address'].toString();;
    permanentAddress = json['permanent_address'].toString() == 'null' ? 'N/A': json['permanent_address'].toString();
  }

  Map<String, dynamic> toJson() {
    final Map<String, dynamic> data = new Map<String, dynamic>();
    data['alt_email'] = this.personalEmail;
    data['email'] = this.officialEmail;
    data['alt_phone'] = this.personalPhoneNo;
    data['phone'] = this.officialPhoneNo;
    data['emergency_contact_name'] = this.emergencyContactPerson;
    data['emergency_contact_relation'] = this.emergencyContactRelation;
    data['emergency_contact_phone'] = this.emergencyContactNumber;
    data['emergency_contact_address'] = this.emergencyContactAddress;
    data['present_address'] = this.presentAddress;
    data['permanent_address'] = this.permanentAddress;
    return data;
  }
}

class PersonalInfo {
  String fathersName;
  String mothersName;
  String gender;
  String bloodGroup;
  String maritalstatus;
  String religion;
  String bankACNo;
  String nIDPassportNo;
  String tinNo;
  String nationality;
  String drivingLicenseNumber;

  PersonalInfo(
      {this.fathersName,
      this.mothersName,
      this.gender,
      this.bloodGroup,
      this.maritalstatus,
      this.religion,
      this.bankACNo,
      this.nIDPassportNo,
      this.tinNo,
      this.nationality,
      this.drivingLicenseNumber});

  PersonalInfo.fromJson(Map<String, dynamic> json) {
    fathersName = json['father_name'].toString() == 'null' ? 'N/A': json['father_name'].toString();
    mothersName = json['mother_name'].toString() == 'null' ? 'N/A': json['mother_name'].toString();
    gender = json['gender'].toString() == 'null' ? 'N/A': json['gender'].toString();
    bloodGroup = json['blood_group'].toString() == 'null' ? 'N/A': json['blood_group'].toString();
    maritalstatus = json['marital_status'].toString() == 'null' ? 'N/A': json['marital_status'].toString();
    religion = json['religion'].toString() == 'null' ? 'N/A': json['religion'].toString();
    bankACNo = "N/A";
    nIDPassportNo = json['national_id'].toString() == 'null' ? 'N/A': json['national_id'].toString();
    tinNo = json['tin_number'].toString() == 'null' ? 'N/A': json['tin_number'].toString();
    nationality = json['nationality'].toString() == 'null' ? 'N/A': json['nationality'].toString();
    drivingLicenseNumber = json['driving_license_number'].toString() == 'null' ? 'N/A': json['driving_license_number'].toString();
  }

  Map<String, dynamic> toJson() {
    final Map<String, dynamic> data = new Map<String, dynamic>();
    data['mother_name'] = this.fathersName;
    data['MothersName'] = this.mothersName;
    data['gender'] = this.gender;
    data['blood_group'] = this.bloodGroup;
    data['marital_status'] = this.maritalstatus;
    data['religion'] = this.religion;
    data['payment']['bank_account_number'] = this.bankACNo;
    data['national_id'] = this.nIDPassportNo;
    data['tin_number'] = this.tinNo;
    data['nationality'] = this.nationality;
    return data;
  }
}

class UpdateProfileInfo {
  String presentAddress;
  String permanentAddress;
  String drivingLicenseNumber;
  String personalEmail;
  String personalPhoneNo;
  String emergencyContactPerson;
  String emergencyContactRelation;
  String emergencyContactNumber;
  String emergencyContactAddress;
  String bloodGroup;


  UpdateProfileInfo(
      {this.personalEmail,this.personalPhoneNo,this.emergencyContactPerson,this.emergencyContactRelation,this.bloodGroup,
        this.emergencyContactNumber,this.emergencyContactAddress,this.presentAddress,this.permanentAddress,this.drivingLicenseNumber});

  Map<String, dynamic> toJson() {
    final Map<String, dynamic> data = new Map<String, dynamic>();
    data['present_address'] = this.presentAddress;
    data['permanent_address'] = this.permanentAddress;
    data['driving_license_number'] = this.drivingLicenseNumber;
    data['alt_email'] = this.personalEmail;
    data['alt_phone'] = this.personalPhoneNo;
    data['emergency_contact_name'] = this.emergencyContactPerson;
    data['emergency_contact_relation'] = this.emergencyContactRelation;
    data['emergency_contact_phone'] = this.emergencyContactNumber;
    data['emergency_contact_address'] = this.emergencyContactAddress;
    data['blood_group'] = this.bloodGroup;

    return data;
  }
}
