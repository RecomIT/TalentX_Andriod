...
class WorkFromHomeApplicationScreen extends StatelessWidget {
  final ApiService apiService;
  final GlobalKey<ScaffoldState> scaffoldKey;
  final User currentUser;
  const WorkFromHomeApplicationScreen({
    Key key,
  ...
  Widget build(BuildContext context) {
    ...
    return
      Column(
        children: [
          //SizedBox(height: 20),
          FutureBuilder<WorkFromHomeList>(
              future: apiService.getWorkFromHomeList(),
              builder: (context, snapshot) {
                if (snapshot.hasData && !snapshot.hasError) {
                  var wfhList = snapshot.data.workFromHomeList.toList();
                  if(wfhList == null || wfhList.length ==0){
                    return Text('Message : No Off-Site Attendance Records Available');
                  }else{
                    wfhList.sort((a,b)=>b.wfhId.compareTo(a.wfhId));
                    return Column(
                      children: List.generate(
                        wfhList?.length,
                            (idx) {
                          return wfhList?.length > 0
                              ? Container(
                                ...
                                SizedBox(
                                  ...
                                  child: Padding(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        Text(
                                          'Reason : ${wfhList[idx].type.capitalize()}', //${_colleagues[idx].code}
                                          style: TextStyle(
                                        ...
                                        SizedBox(height: 5,),
                                        Text(
                                          'Start Date : ${wfhList[idx].startDate}', //${dateFormatter.format(DateTime.tryParse(wfhList[idx].startDate))}',
                                          style: TextStyle(
                                            color: Colors.black45,
                                            fontWeight: FontWeight.bold,
                                            fontSize: 12,
                                        ...
                                        SizedBox(height: 5,),
                                        Text(
                                          'End Date : ${wfhList[idx].endDate}', //${dateFormatter.format(DateTime.tryParse(wfhList[idx].endDate))}', //
                                          style: TextStyle(
                                            color: Colors.black45,
                                            fontWeight: FontWeight.bold,
                                            fontSize: 12,
                                        ...
                                        SizedBox(height: 5,),
                                        Text(
                                          'No. of Days/Hours : ${wfhList[idx].numberOfDays}', //${_colleagues[idx].code}
                                          style: TextStyle(
                                            color: Colors.black45,
                                            fontWeight: FontWeight.bold,
                                            fontSize: 12,
                                        ...
                                        Text(
                                          'Status : ${wfhList[idx].status.capitalize()}', //${_colleagues[idx].code}
                                          style: TextStyle(
                                            color: Colors.black45,
                                            fontWeight: FontWeight.bold,
                                            fontSize: 12,
